...
beatles$minor_pct <- beatles$roman %>% 
                          str_split('-') %>% 
                          lapply(function(x) sum(str_detect(x, 'i|v') ))  %>% 
                          unlist
beatles$song
#### Plot Percent Borrowed Chords ####
pct_chord_tab <-beatles %>% 
                              mutate(wt_pct_borrowed =  borrowed_pct*num_chords,
                                     wt_pct_minor =  minor_pct*num_chords )%>% 
                              drop_na(album_name) %>% 
                              group_by(song, phase) %>% 
                              summarize(pct_borrowed_sum = sum(wt_pct_borrowed) / sum(num_chords),
                                        pct_minor_sum = sum(wt_pct_minor) / sum(num_chords))
  

pct_chord_tab %>% 
  ggplot(aes(x = pct_minor_sum, y = pct_borrowed_sum, color = phase )) +
  geom_point()
