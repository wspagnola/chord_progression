...
#### Research Question
How did the chord progressions in Beatles songs evolve over time? 
...
As I have worked on this project, I have noticed that it very difficult to describe its relevance because music is a very abstract thing.  If this were a research project on the use of color in paintings, for example, it would be more accessible because although many people may not have a deep understanding of color theory, most have at least an understanding of colors.  They know what 'blue', 'red', and 'yellow' mean.  However, even people who listen to a lot of music would probably not understand what a 'I-IV-V' or 'ii-V-I' chord progression means unless they have taken a class in music theory.  In addition, it is hard to express the importance of understanding how chords go together in a sequence unless you have at one point sat down and tried to write a song.

In other words, my intended audience would be someone who already has a background in music theory and would be able to understand a basic chord progression written in roman numerical analysis.  My belief is that understanding how chord progressions in the music of Beatles changed over time will help musical historians help to understand the compositional structure of their songs and will help musicians writing songs understand unique qualities in Beatles music that may inform their own compositions. 
...
When I was growing up, I was always fascinated by the two images from the Beatles first two compilation albums.  The red album contains songs from 1962 to 1966 and the blue album contains songs from 1967 to 1970.  However, the actual cutoff date  is June 21, 1966.  The cover of album shows the fab four in the same location, but with wildly contrasting images between their clean cut mop-top days and their laster long-haired psychodelic phase.  My goal with this project is to examine the stylistic progression of the Beatles by examining their most common chord progression during each phase.  


#### Literature Review
I found two particularly relevant articles.  Douglas J. Mason, a Ph. D. student at Harvard, used hierarchical clustering techniques to examine chords in Beatles songs (2012).  He noticed certain clusters of songs based on the usage of 'borrowed chords', which are chords that do not belong to the key of the song but are in fact 'borrowed' from a different scale.  These clusters generally contained songs that were from different albums.  However, the majority of the songs, which were taken from *Rolling Stone's* list of the Top 100 Beatles songs, seem to be from their later period (1967  to 1970).

In another study, Pérez-Sancho et. al employed text analysis techiniques such as bigrams and trigrams to build models for classifying musical pieces according to genre (2009).  Using this method, they were able to classify songs according to genre with 80% accuracy.  However, they did not achieve as high of accuracy when classifying music according to sub-genres (e.g. classifying different types of music within the rock genre).


#### Data Collection 
I built a webcrawler to scrape chord from Hooktheory.com.  Hooktheory contains over 12,000 songs. Each song is  divided according to different sections (e.g. Verse, Chorus, etc.).  The site does not contain the entire song but rather just the repeating pattern for each section.  Below, you can see the chords from the  verse from 'Hey Jude', which are repeated throughout the song. I was able to scrape the 'letter' chords but no the roman numerical analysis.  However, using the song key, which I also scraped from the site, I was able to build a function that converted the 'letter' chords into roman numerals.  As I will explain in the next session, the roman numerals represent each chord's position in a given key. 
...
Using roman numerical analysis, we can compare equivalent chord progressions in different keys.  For example, let's say we want to write a song in 'G major'.  Then a G major chord  would now be represented by 'I' and 'D major' would be represented by 'V' because it would be the fifth chord in the key of G (G, A, B, C, D, E, and F#)
...
There were 37 songs available from the early phase and 63 songs from the later phase.  There were a total of 100 songs, which is the same asthe sample size from the Beatles Genome {roject. 
